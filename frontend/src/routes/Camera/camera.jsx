  ...
  const captureImage = async () => {
    ...
    if (video) {
      ...
      EXIF.getData(blob, async function() {
        ...
        let geoLocation = allMetaData.GPSLatitude && allMetaData.GPSLongitude
        ...
        reader.onloadend = function() {
          setImages(prevImages => [...prevImages, { src: reader.result, geoLocation, dateTime: DateTimeOriginal }]);
        }
    ...
    <div>
      ...
      {images.map((image, index) => (
        <div key={index}>
          ...
          <div>Geolocation: {image.geoLocation}</div>
          <div>Date and Time: {image.dateTime}</div>
        </div>
      ))}
    </div>
  );
